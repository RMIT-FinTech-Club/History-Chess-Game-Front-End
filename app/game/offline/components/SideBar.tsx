...
const SideBar = ({ isOpen }: { isOpen: boolean }) => {
  ...
  return (
    <div
      className={`fixed top-0 right-0 z-40 h-[100dvh] min-w-[calc(6vw+5vh)] bg-black flex flex-col items-center transform transition-transform duration-300 ${isOpen ? "translate-x-0" : "translate-x-full"
        }`}
    >
      <div className="w-full my-auto flex flex-col items-center justify-center text-white space-y-[1vh]">
        <Link
          href="/home"
          className="flex items-center my-[2vh] gap-[1vh] font-bold text-[3vh] hover:text-gold-hover hover:scale-105 transition-all duration-200"
        >
          <Home size="4vh" />
          Home
        </Link>
      ...
      <Link
        href="/profile"
        className="py-[2vh] px-[2vh] rounded-t-[2vh] bg-gold-dark mt-auto flex items-center justify-between cursor-pointer hover:brightness-110 transition-all duration-200"
      >
        <div
